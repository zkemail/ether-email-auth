...
abstract contract EmailAccountRecovery {
    ...
    function acceptGuardian(address guardian, uint templateIdx, bytes[] memory subjectParams, bytes32 emailNullifier) public virtual;
    function recoverWallet(address guardian, uint templateIdx, bytes[] memory subjectParams, bytes32 emailNullifier) public virtual;

    function handleAcceptance(EmailAuthMsg memory emailAuthMsg, uint templateIdx) external {
        address guardian = Create2.computeAddress(emailAuthMsg.proof.accountSalt, bytes32(bytes20(emailAuthImplementation())));
        require(Address.isContract(guardian), "guardian is not deployed");
        uint templateId = uint256(keccak256(abi.encode(EMAIL_ACCOUNT_RECOVERY_VERSION_ID, "ACCEPTANCE", templateIdx)));
        require(templateId == emailAuthMsg.templateId, "invalid template id");
        require(emailAuthMsg.proof.isCodeExist == true, "isCodeExist is false");

        // Deploy proxy 
        ERC1967Proxy proxy = new ERC1967Proxy(address(emailAuthImplementation()), abi.encode(dkim(), verifier()));
        ...
        for (uint i = 0; i < acceptanceSubjectTemplates().length; i++) {
            guardianEmailAuth.insertSubjectTemplate(i, acceptanceSubjectTemplates()[i]);
        }
    ...
    function handleRecovery(EmailAuthMsg memory emailAuthMsg, uint templateIdx) external {
        address guardian = Create2.computeAddress(emailAuthMsg.proof.accountSalt, bytes32(bytes20(emailAuthImplementation())));
        require(Address.isContract(guardian), "guardian is not deployed");
        uint templateId = uint256(keccak256(abi.encode(EMAIL_ACCOUNT_RECOVERY_VERSION_ID, "RECOVERY", templateIdx)));
        require(templateId == emailAuthMsg.templateId, "invalid template id");
        ...
        recoverWallet(guardian, templateIdx, emailAuthMsg.subjectParams, emailAuthMsg.proof.emailNullifier);
    }
}
