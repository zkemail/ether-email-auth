...
template AccountCodeWithPrefixRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[2][i].in[12] <== eq[1][i].out;
		multi_or[2][i].in[13] <== eq[4][i].out;
		multi_or[2][i].in[14] <== eq[5][i].out;
	...
	for (var i = 0; i < msg_bytes; i++) {
		...
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][0] * states[i+2][2];
		is_substr0[i][3] <== is_substr0[i][2] + states[i+1][1] * states[i+2][3];
		...
		is_substr0[i][8] <== is_substr0[i][7] + states[i+1][5] * states[i+2][7];
		is_substr0[i][9] <== is_substr0[i][8] + states[i+1][6] * states[i+2][7];
		is_substr0[i][10] <== is_substr0[i][9] + states[i+1][7] * states[i+2][7];
