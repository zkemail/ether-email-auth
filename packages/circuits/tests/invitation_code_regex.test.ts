...
describe("Invitation Code Regex", () => {
  it("invitation code", async () => {
    ...
    const circuit = await wasm_tester(
      path.join(__dirname, "./circuits/test_account_code_regex.circom"),
      option
    );
  ...
  it("invitation code in the subject", async () => {
    ...
    expect(1n).toEqual(witness[1]);
    const prefixIdxes = emailWalletUtils.extractInvitationCodeIdxes(codeStr)[0];
    // const revealedStartIdx = emailWalletUtils.extractSubstrIdxes(codeStr, readFileSync(path.join(__dirname, "../src/regexes/account_code.json"), "utf8"))[0][0];
    // console.log(emailWalletUtils.extractSubstrIdxes(codeStr, readFileSync(path.join(__dirname, "../src/regexes/account_code.json"), "utf8")));
    for (let idx = 0; idx < 256; ++idx) {
      if (idx >= prefixIdxes[0] && idx < prefixIdxes[1]) {
        expect(BigInt(paddedStr[idx])).toEqual(witness[2 + idx]);
      } else {
